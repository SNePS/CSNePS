...
\chapter{Running CSNePS}
CSNePS is implemented in the Clojure programming language, a recently developed 
dialect of lisp witch runs (primarily) on the Java Virtual Machine.
...
The preferred method of running CSNePS at the command line is to use the REPL 
component of Leiningen.

\begin{enumerate}
  \item At the command prompt, change directories to the CSNePS directory. 
...
\begin{enumerate}
  ...
  This is the directory containing the project.clj file which Leiningen will use to 
  load the project.
  \item Type \texttt{lein run} at the command prompt and hit enter. This will load CSNePS and its GUI.
\end{enumerate}
...
%Your .emacs 
%file may be configured to run Clojure as an inferior lisp as you would any other, for example:
%
%\begin{verbatim}
%(setq inferior-lisp-program "lein repl"
%    inferior-lisp-load-command "(load \"%s\")\n"
...
%\end{verbatim}
%
%This method is pretty standadrd among lisps and won't be discussed further here. We'll 
%describe the (currently) preferred method of using SLIME and Swank to interact with Clojure. 
%
%\begin{itemize}
%  \item Either create a directory or change to a directory where you will keep Emacs
%  plugins. We'll call it \texttt{/home/user/tools} here, but you can use whatever 
%  you like.
%  \item Clone SLIME to the current directory by executing\\
%    \texttt{git clone https://github.com/technomancy/slime.git}. Note this is not the version
%    of SLIME which comes with current versions of Emacs!
%  \item Clone clojure-mode to the current directory by executing\\
%    \texttt{git clone https://github.com/technomancy/clojure-mode/}.
%  \item If you are using Leiningen 1.x, change directories to the Sneps3-Clojure directory, 
%  then run the command \texttt{lein plugin install swank-clojure 1.4.2}. This is unneccesary for
%  Leiningen 2.x (currently in beta).
%  \item Update your .emacs file with the following:\\
%  \begin{verbatim}
%;; Load and configure SLIME
%(add-to-list 'load-path "/home/user/tools/slime")
...
\begin{enumerate}
  \item Load Emacs and switch the current directory to the Sneps3-Clojure directory.
  This is the directory containing the project.clj file which will be used to 
  load the project.
  \item Execute the \texttt{run-clojure} function we defined in our .emacs file, 
  \texttt{M-x run-clojure}. Any required 
  dependencies will be downloaded, Clojure will load, and you will be presented a 
  ...
  \item You may now load CSNePS by typing \texttt{(loadsneps3)} at the prompt and 
  press enter.
  \item CSNePS is now loaded, follow the directions presented and switch to the 
  snuser namespace by typing \texttt{(in-ns 'snuser)} and pressing enter. CSNePS 
